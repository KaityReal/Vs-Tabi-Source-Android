...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		else if (SONG.song.toLowerCase() == 'genocide')
		{
			defaultCamZoom = 0.8;
			curStage = 'genocide';
			/*var bg:FlxSprite = new FlxSprite(-600, -300).loadGraphic(Paths.image('tabi/mad/youhavebeendestroyed', 'curse'));
			bg.antialiasing = true;
			bg.scrollFactor.set(0.9, 0.9);
			bg.active = false;
			add(bg);
		...
		for (i in 0...4)
		{
			...
			switch (curStage)
			{
				...
				default:
					if (isGenocide)
					{
						babyArrow.frames = Paths.getSparrowAtlas('tabi/mad/NOTE_assets', 'curse');
					} else {
